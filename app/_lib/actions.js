...
export async function updateGuest(formData) {
  ...
  const nationalIdRegex = /^[A-Za-z0-9]{6,14}$/;
...
export async function deleteBooking(bookingId) {
  ...
  if (!guestBookingsId.includes(bookingId))
    throw new Error("You are not allowed to delete this guest bookings");
  ...
  revalidatePath("/bookings/reservations");
}
...
export async function createBooking(bookingData, formData) {
  ...
  // check no booking for the cabin in the selected date
  const bookedDates = await getBookedDatesByCabinId(bookingData.cabinId);

  if (
    bookedDates.some(
      (date) =>
        isSameDay(date, bookingData.startDate) ||
        isSameDay(date, bookingData.endDate)
    )
  )
